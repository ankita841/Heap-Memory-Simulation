...
void* custom_malloc(size_t size)
{
    ...
    if (head == NULL)
    {
        ...
        head->data = head + (sizeof(struct block)); // data will be in memory addresses directly after block metadata
        ...
        while (block_size != request_size)
        {
            ...
            split_block = head + (int)block_size; // always split from head onwards
    ...
    else
    {
        ...
        while (curr)
        {
            ...
            {
                curr->free = false;
                curr->data = curr + (sizeof(struct block));
                return_block = curr + (sizeof(struct block));
                return return_block;
            }
            else if (curr->free == true && curr->size > request_size)
            {
                ...
                while (block_size != request_size)
                {
                    ...
                    split_block = curr + (int)block_size;
                    split_block->size = block_size;
                    split_block->next = curr->next;
                    split_block->buddy = ((split_block)-((int)block_size));
                    split_block->free = true;
...
void* custom_realloc(void* ptr, size_t size)
{
    ...
    return dest + sizeof(struct block);
}
